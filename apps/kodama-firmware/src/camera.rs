//! Camera mode: captures video, audio, and telemetry from Pi camera and streams to server.
...
async fn run_async() -> Result<()> {
    ...
    let video_rx = if config.test_source {
        #[cfg(feature = "test-source")]
        {
            ...
            kodama::capture::start_test_source(kodama::capture::TestSourceConfig {
...
async fn execute_update_firmware(params: UpdateFirmwareParams) -> CommandResult {
    ...
    match super::update::UpdateState::new(params.url, params.sha256) {
        Ok(state) => {
            tokio::spawn(async move {
